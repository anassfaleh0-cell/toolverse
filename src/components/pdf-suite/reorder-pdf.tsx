  ...
  if (!file) {
    return (
      <div className="mx-auto max-w-3xl">
        ...
        >
          <svg className="mb-3 size-10 text-zinc-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5M12 17.25h8.25" /></svg>
        ...
        {order.map((pageIndex, pos) => (
          <div key={pos} className="space-y-2 rounded-lg border border-zinc-200 p-3 dark:border-zinc-800">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-zinc-600">#{pos + 1}</span>
            </div>
            <div className="flex items-center justify-center rounded bg-zinc-50 py-4 dark:bg-zinc-900">
              <svg className="size-10 text-zinc-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg>
            </div>
            <div className="flex justify-center gap-1">
              <Button variant="ghost" size="sm" onClick={() => movePage(pos, "up")} disabled={pos === 0} aria-label="Move page up">â†‘</Button>
              <Button variant="ghost" size="sm" onClick={() => movePage(pos, "down")} disabled={pos === order.length - 1} aria-label="Move page down">â†“</Button>
            </div>
          </div>
        ))}
      </div>
