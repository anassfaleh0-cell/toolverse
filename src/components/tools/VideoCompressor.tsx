...
export function VideoCompressor() {
  ...
  async function handleCompress() {
    ...
    try {
      ...
      const w = video.videoWidth * (1 - (1 - quality) * 0.5);
      const h = video.videoHeight * (1 - (1 - quality) * 0.5);
  ...
  const ratio = file ? ((1 - resultSize / file.size) * 100).toFixed(1) : "0";
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="rounded-xl border-2 border-dashed border-zinc-300 p-8 text-center dark:border-zinc-600">
        ...
        {file ? (
          ...
          <button onClick={() => inputRef.current?.click()} className="cursor-pointer text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100">
            ...
            <p className="text-xs text-zinc-400 mt-1">MP4, AVI, MOV, WebM</p>
          </button>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        <input type="range" min={0.1} max={1} step={0.05} value={quality} onChange={(e) => setQuality(parseFloat(e.target.value))} className="w-full accent-nuvora-600" />
        <div className="flex justify-between text-xs text-zinc-500"><span>Smaller size</span><span>Higher quality</span></div>
      </div>

      <Button onClick={handleCompress} disabled={!file || loading} variant="primary">
      ...
      {result && (
        <Card variant="default" className="p-5">
          <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100">Compression complete</p>
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Reduced by {ratio}% ({(resultSize / 1024 / 1024).toFixed(2)} MB)</p>
          <a href={result} download={`compressed-${file?.name ?? "video.webp"}`} className="mt-3 inline-flex items-center gap-2 rounded-lg bg-nuvora-600 px-4 py-2 text-sm font-medium text-white hover:bg-nuvora-700">
