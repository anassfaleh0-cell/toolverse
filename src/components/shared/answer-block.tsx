      ...
      <section className="rounded-xl border border-zinc-200 dark:border-zinc-800">
        <div className="border-b border-zinc-200 bg-zinc-50 px-5 py-4 dark:border-zinc-800 dark:bg-zinc-900">
          <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
            {toolName ? `${toolName} â€” ` : ""}Quick Answer
          </p>
        </div>
        <div className="px-5 py-4">
            ...
            <ul className="mt-3 space-y-1">
              {bulletPoints.map((point, i) => (
                <li key={i} className="flex items-start gap-2 text-sm text-zinc-600 dark:text-zinc-400">
                  <svg viewBox="0 0 16 16" fill="currentColor" className="mt-0.5 size-4 shrink-0 text-blue-700" aria-hidden="true">
          ...
          {codeExample && (
            <pre className="mt-3 overflow-x-auto rounded-lg bg-zinc-900 p-4 text-xs text-zinc-600 dark:bg-black">
