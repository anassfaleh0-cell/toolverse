...
export const metadata: Metadata = {
  title: `DNS Record Types â€” Complete Reference`,
...
const DNS_RECORD_TYPES: DnsRecordType[] = [
  { type: "A", fullName: "Address Record", purpose: "Maps a domain name to an IPv4 address.", format: "example.com.  IN  A  192.0.2.1", example: "example.com â†’ 192.0.2.1", whenToUse: "Every website or service that needs to be reachable via IPv4.", rfc: "RFC 1035" },
  { type: "AAAA", fullName: "IPv6 Address Record", purpose: "Maps a domain name to an IPv6 address.", format: "example.com.  IN  AAAA  2001:db8::1", example: "example.com â†’ 2001:db8::1", whenToUse: "Every website or service that needs to be reachable via IPv6. Essential for modern networks.", rfc: "RFC 3596" },
  { type: "CNAME", fullName: "Canonical Name Record", purpose: "Aliases one domain name to another. The target domain must have its own A/AAAA record.", format: "www.example.com.  IN  CNAME  example.com.", example: "www.example.com â†’ example.com", whenToUse: "Subdomain aliasing (www â†’ root), CDN integration, load balancing across domains.", rfc: "RFC 1035" },
  ...
  { type: "PTR", fullName: "Pointer Record", purpose: "Maps an IP address to a domain name (reverse DNS lookup).", format: "1.2.0.192.in-addr.arpa.  IN  PTR  example.com.", example: "192.0.2.1 â†’ example.com", whenToUse: "Email server identification (anti-spam), network troubleshooting, logging, server identification.", rfc: "RFC 1035" },
...
export default function DnsRecordTypesPage() {
  return (
    <>
      <JsonLd data={webPageSchema({ name: `DNS Record Types Reference â€” ${SITE_NAME}`, description: `Complete reference of all DNS record types.`, url: `${SITE_URL}/dns-record-types`, breadcrumbs })} />
              ...
              <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                {DNS_RECORD_TYPES.map((record) => (
                  <tr key={record.type} className={`border-l-4 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 ${getTypeColor(record.type)}`}>
                    ...
                    <td className="hidden max-w-xs truncate px-4 py-4 font-mono text-xs text-zinc-500 dark:text-zinc-400 lg:table-cell">{record.format}</td>
                    <td className="hidden px-4 py-4 text-xs text-zinc-500 dark:text-zinc-400 lg:table-cell">{record.whenToUse}</td>
                  </tr>
                ))}
              </tbody>
            </table>
