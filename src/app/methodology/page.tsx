...
export const metadata: Metadata = {
  title: `How We Test and Review Tools â€” Our Methodology | ${SITE_NAME}`,
  description: `${SITE_NAME} explains how our free online tools are built, tested, and maintained â€” including tool selection, testing methodology, data sources, privacy, accuracy, and update frequency.`,
...
const sections = [
  ...
  },
  {
    ...
    content: [
      ...
      "IP geolocation data is sourced from ip-api.com and cross-validated against Regional Internet Registry (RIR) data from ARIN, RIPE NCC, APNIC, LACNIC, and AFRINIC. WHOIS lookups query registry WHOIS servers and RDAP endpoints directly â€” we do not use third-party WHOIS aggregators or caches.",
  ...
  },
  {
    ...
    content: [
      ...
      "Network diagnostic tools that rely on real-time queries (DNS Lookup, Ping Test, SSL Certificate Checker) are updated continuously by nature â€” each query returns current data. The tool implementations themselves are reviewed quarterly to ensure compatibility with evolving protocols and standards.",
...
export default function MethodologyPage() {
  return (
    <>
      <JsonLd data={webPageSchema({ name: `How We Test and Review Tools â€” Our Methodology | ${SITE_NAME}`, description: `${SITE_NAME}'s rigorous approach to building, testing, and maintaining free online tools.`, url: `${SITE_URL}/methodology`, breadcrumbs })} />
      ...
      <section className="border-b border-zinc-200 bg-zinc-50 py-12 dark:border-zinc-800 dark:bg-zinc-900/50 sm:py-16">
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          ...
          <div className="mt-6">
            <SocialShare url={`${SITE_URL}/methodology`} title={`How We Test and Review Tools â€” Our Methodology | ${SITE_NAME}`} />
          </div>
        </div>
      </section>
            ...
            <ul className="mt-2 space-y-1">
              {sections.map((section) => (
                <li key={section.id}>
                  <a
                    href={`#${section.id}`}
                    className="text-sm text-zinc-600 hover:text-blue-600 dark:text-zinc-400 dark:hover:text-blue-700"
                  >
                    {section.title}
                  </a>
                </li>
