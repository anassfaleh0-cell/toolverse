...
const steps = {
  ...
  certExpiredFix: {
    ...
    yesFix: "Renew your SSL certificate with your Certificate Authority immediately. If using Let's Encrypt, run certbot renew. For other CAs, follow their renewal process. Install the new certificate on your server and verify it using the SSL Certificate Checker (toolverse.dev/ssl-certificate-checker). Set up automatic renewal or calendar reminders to avoid future expirations.",
  },
  hostnameMismatchFix: {
    id: "hostnameMismatchFix",
    isYesFix: true,
    checkLabel: "Fix hostname match",
    yesFix: "The certificate does not cover the domain name being accessed. Obtain a new certificate that includes the correct domain in the Subject Alternative Names (SANs) list. If you manage multiple subdomains, consider a wildcard certificate (*.example.com). Note that wildcards only cover one level of subdomain. Verify the fix with the SSL Certificate Checker (toolverse.dev/ssl-certificate-checker).",
  },
  installIntermediatesFix: {
    id: "installIntermediatesFix",
    isYesFix: true,
    checkLabel: "Install intermediate certificates",
    yesFix: "Download the CA bundle (chain certificate) from your Certificate Authority and install it on your server. For Nginx, concatenate the intermediates with your certificate file. For Apache, use SSLCertificateChainFile or SSLCACertificateFile. For IIS, import the intermediate certificates into the server's certificate store. Verify the complete chain with the SSL Certificate Checker (toolverse.dev/ssl-certificate-checker).",
  },
  checkCaTrustFix: {
    id: "checkCaTrustFix",
    isYesFix: true,
    checkLabel: "Check certificate chain",
    yesFix: "If the chain is complete but the CA is still untrusted, your CA may not be widely trusted, or the root certificate is missing from the client's trust store. Ensure you are using a CA that is included in major browser trust stores (e.g., Let's Encrypt, DigiCert, Sectigo, GlobalSign). For private/internal CAs, distribute your root CA certificate to all clients. Use the SSL Certificate Checker (toolverse.dev/ssl-certificate-checker) to inspect the full chain.",
  },
  mixedContentFix: {
    id: "mixedContentFix",
    isYesFix: true,
    checkLabel: "Fix mixed content",
    yesFix: "Update all resource URLs on your HTTPS pages to use HTTPS. Check for hardcoded http:// URLs in scripts, stylesheets, images, fonts, and API calls. Use relative protocol URLs (//example.com/file.js) or update to https://. Browser developer tools (F12 > Console > Mixed Content) can identify specific resources. Use the HTTP Headers Checker (toolverse.dev/http-headers-checker) to verify Content-Security-Policy headers.",
  },
  tlsUpgradeFix: {
    id: "tlsUpgradeFix",
    isYesFix: true,
    checkLabel: "Upgrade TLS protocol",
    yesFix: "Disable TLS 1.0 and TLS 1.1 on your server. Enable only TLS 1.2 and TLS 1.3 for maximum security and compatibility. Check your web server configuration: for Nginx, set ssl_protocols TLSv1.2 TLSv1.3; for Apache, set SSLProtocol -all +TLSv1.2 +TLSv1.3. Verify the changes with the SSL Certificate Checker (toolverse.dev/ssl-certificate-checker) which reports supported protocols.",
  },
  generalSslCheck: {
    id: "generalSslCheck",
    isYesFix: true,
    checkLabel: "Run SSL Certificate Checker",
    yesFix: "No obvious issues found from the guided questions. Run a comprehensive scan using the SSL Certificate Checker (toolverse.dev/ssl-certificate-checker). It will validate the certificate chain, check revocation status, verify supported TLS protocols, inspect cipher suites, and identify any misconfigurations. Review the TLS Versions Guide (toolverse.dev/tls-versions) for best practices.",
  },
  noIssueFix: {
    id: "noIssueFix",
    isYesFix: true,
    checkLabel: "Check SSL Certificate Checker",
    yesFix: "No SSL issues detected from your feedback. If problems persist, run the SSL Certificate Checker (toolverse.dev/ssl-certificate-checker) for a detailed audit. Common issues that don't show browser warnings include weak cipher suites, certificate revocation status problems, and OCSP stapling misconfigurations.",
  },
  otherErrors: {
    id: "otherErrors",
    isYesFix: true,
    checkLabel: "Run SSL Certificate Checker",
    yesFix: "Your SSL error type was not covered in detail by this flow. Use the SSL Certificate Checker (toolverse.dev/ssl-certificate-checker) to perform a comprehensive analysis of your certificate. Check for revoked certificates, OCSP stapling issues, certificate transparency (CT) requirements, or HPKP pinning failures. The checker will identify the specific problem and suggest a resolution.",
  },
};
